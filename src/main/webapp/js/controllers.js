...
function PianoCtrl($http, $scope) {

	$http.defaults.headers.post['Content-Type'] = 'application/json';

	
	var genKeyboard = function(startOctave, endOctave) {
		if (startOctave < 0 || endOctave > 8 || startOctave >= endOctave)
			throw "Bad arguments";
		var notes = [ [ 'C', 0, 'white' ], [ 'Db', 0.5, 'black' ],
				[ 'D', 1, 'white' ], [ 'Eb', 1.5, 'black' ],
				[ 'E', 2, 'white' ], [ 'F', 3, 'white' ],
				[ 'Gb', 3.5, 'black' ], [ 'G', 4, 'white' ],
				[ 'Ab', 4.5, 'black' ], [ 'A', 5, 'white' ],
				[ 'Bb', 5.5, 'black' ], [ 'B', 6, 'white' ], ];
			...
			for ( var i = start; i < end; i++) {
				keys
						.push({
							...
							cssPosition : {
								left : (100 * (notes[i][1] - 5 + (oct - startOctave) * 7) / totalWhites)
										+ '%', // 5:A first, 7:whites/octave
	...
	$scope.sendMidiCommand = function(command, midiNote) {
		...
		$http.post('data/midi/send', midiMessage).success(
